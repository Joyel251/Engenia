...
class CarLights {
  ...
  update(time: number) {
    if ((this.mesh.material as any).uniforms.uTime) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {}, boost = false }) => {
  ...
  // Respond to boost changes by adjusting internal speed/FOV targets
  useEffect(() => {
    if (appRef.current) {
      const app = appRef.current as any
      app.fovTarget = boost ? mergedOptions.fovSpeedUp : mergedOptions.fov
      app.speedUpTarget = boost ? mergedOptions.speedUp : 0
    }
  }, [boost, mergedOptions.fovSpeedUp, mergedOptions.fov, mergedOptions.speedUp])
